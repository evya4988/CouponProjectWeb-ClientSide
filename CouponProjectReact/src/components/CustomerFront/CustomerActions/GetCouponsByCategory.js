...
const GetCouponsByCategory = (props) => {
    ...
    const fetchCouponsByCategoryHandler = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch("http://localhost:8080/company/getCompanyCouponsByCategory/" + category, requestOptions);
            if (!response.ok) {
    ...
    }, [dispatch, token, props, selectedCategory, companyId]);
