...
const GetCouponsByMinPrice = (props) => {
    ...
    const fetchCouponsByMinPriceHandler = useCallback(async () => {
        ...
        const requestOptions = {
            method: "GET",
            headers: { "Content-Type": "application/json", minPrice: minPrice, companyId: companyId, token: token },
        };

        try {
            console.log("get all coupons!");
            const response = await fetch("http://localhost:8080/company/getCompanyCoupons/MinPrice", requestOptions);
            if (!response.ok) {
