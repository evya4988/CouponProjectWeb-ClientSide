...
const GetCouponsByMaxPrice = (props) => {
    ...
    const fetchCouponsByMaxPriceHandler = useCallback(async () => {
        ...
        const requestOptions = {
            method: "GET",
            headers: { "Content-Type": "application/json", maxPrice: maxPrice, id: id, token: token },
        };

        try {
            console.log("get all coupons!");
            const response = await fetch("http://localhost:8080/"+ role +"/get"
            + role.slice(0,1).toUpperCase() + role.slice(1, role.length) +"Coupons/MaxPrice", requestOptions);
            if (!response.ok) {
